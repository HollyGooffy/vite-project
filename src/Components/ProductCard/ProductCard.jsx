            ...
            <div className="pt-2 pb-3">
                ...
                <div className="flex items-start gap-1 mb-2 flex-col">
                    {product.oldPrice && product.oldPrice > product.price && (
                        <span className="text-gray-400 text-[9px] font-light line-through">{product.oldPrice} ₽/кг</span>
                    )}
                    <span className="font-semibold text-xs">{product.price} ₽/кг</span>

                </div>

                <Button text="В корзину" className="py-1.5 !px-8.5 font-semibold whitespace-nowrap text-center"/>
            </div>
        </div>
    );
};
